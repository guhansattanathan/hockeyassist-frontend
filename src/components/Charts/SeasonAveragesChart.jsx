...
const SeasonAveragesChart = ({ seasons }) => {
  ...
  if (!seasons || seasons.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 bg-black border border-zinc-800 rounded-2xl text-zinc-500">
        <p className="text-sm font-medium">No per-game averages available</p>
      </div>
    );
  }

  // Map & sanitize data returned from getPlayerSeasonsAverages endpoint
  const data = [...seasons]
    .sort((a, b) => (a.seasonId || a.season || '').localeCompare(b.seasonId || b.season || ''))
    .map((season) => {
      const rawPpg = season.ppg ?? season.pointsPerGame ?? season.pts ?? season.points ?? 0;
      const rawRpg = season.rpg ?? season.reboundsPerGame ?? season.reb ?? season.rebounds ?? 0;
      const rawApg = season.apg ?? season.assistsPerGame ?? season.ast ?? season.assists ?? 0;

      return {
        season: season.seasonId || season.season || 'N/A',
        ppg: parseFloat(Number(rawPpg).toFixed(1)),
        rpg: parseFloat(Number(rawRpg).toFixed(1)),
        apg: parseFloat(Number(rawApg).toFixed(1)),
      };
    });
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-xs text-zinc-400 mt-0.5">
            Season-by-season breakdown for Points, Rebounds, and Assists per game
          </p>
        </div>
      ...
      <ResponsiveContainer width="100%" height={340}>
        <AreaChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
          <defs>
            ...
            <linearGradient id="rpgGrad" x1="0" y1="0" x2="0" y2="1">
...
export default SeasonAveragesChart;